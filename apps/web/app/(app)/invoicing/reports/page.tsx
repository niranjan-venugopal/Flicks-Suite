...
/** Reports hub — prototype ScrInvReports (KPI cards + aging + GSTR-1). */
export default function InvReportsPage() {
  const { toast } = useToast()
  const { data: aging } = useAging()
  const { data: dash } = useInvDashboard()
  ...
  const onExport = async () => {
    try {
      ...
      // download the JSON file (R2 upload joins later — the payload is authoritative)
  ...
  const kpis = [
    ...
    { t: 'TDS receivable', d: `${inr(tds?.meta.total ?? 0)} across ${tds?.meta.count ?? 0} invoices`, icon: Calculator, c: 'var(--blue)', cta: 'Section 393', onClick: undefined },
    { t: 'Aging report', d: `${inr(total)} outstanding`, icon: Clock, c: 'var(--yellow)', cta: 'Below', onClick: undefined },
    { t: 'Collected', d: `${inr(dash?.data.collected ?? 0)} · ${dash?.data.paid ?? 0} paid`, icon: RefreshCw, c: 'var(--green)', cta: 'MRR in Sprint 7', onClick: undefined },
  ]

  return (
    <div style={{ padding: '26px 28px 72px' }}>
      <SectionHead title="Invoicing reports" sub="GSTR-1 · TDS · receivables aging — Tally & GST-portal compatible exports." />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        ...
        <div className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <div className="t-h3">Receivables aging</div>
            <Pill tone="blue">{inr(total)}</Pill>
          </div>
          {total > 0 && (
          ...
          {buckets.map((b, i) => (
            <div key={b.bucket} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 0', borderBottom: '1px solid var(--bord)' }}>
              ...
              <span className="t-num" style={{ fontSize: 13, fontWeight: 800 }}>{inr(b.amount)}</span>
            </div>
          ))}
        </div>
              ...
              <tbody>
                {([
                  ...
                  <tr key={i}>
                    ...
                    <td className="t-num" style={{ textAlign: 'right', fontWeight: 700 }}>{inr(r[2])}</td>
                    <td className="t-num" style={{ textAlign: 'right', fontWeight: 700 }}>{r[3] === '—' ? '—' : inr(r[3])}</td>
                  </tr>
                ))}
              </tbody>
            </table>
