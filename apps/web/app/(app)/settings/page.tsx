...
import { useCheckSlug } from '@/lib/api/queries/use-onboarding'
import { useDebounce } from '@/lib/hooks/use-debounce'
import { stateCodeFromGstin, stateName } from '@flicks/shared/constants'
...
import { COUNTRIES, IN_STATE_CODES as STATE_CODES, TIMEZONES } from '@/lib/countries'
...
// Mirrors the signup wizard's derivation — a rename suggests the matching ID.
...
export default function OrganizationSettingsPage() {
  ...
  // Workspace-wide preferences (round 13/14): saved through the same
  // organization endpoint, shown as one more card on this page.
  const [prefs, setPrefs] = useState<Prefs>({ timezone: 'Asia/Kolkata', fiscalYearStartMonth: 4, weekStartsOn: 1 })
  const [basePrefs, setBasePrefs] = useState<Prefs>(prefs)
  ...
  const dirty = useMemo(
    () =>
      ...
      JSON.stringify(prefs) !== JSON.stringify(basePrefs),
    [form, baseline, prefs, basePrefs],
  )

  // ── Editable slug (round 13): availability checked live (reserved names +
  // global uniqueness, same rules as signup); renaming the workspace
  // auto-suggests the matching ID until the slug is edited by hand.
  const checkSlug = useCheckSlug()
  const [slugTouched, setSlugTouched] = useState(false)
  const [slugAvailable, setSlugAvailable] = useState<boolean | null>(null)
  const slugChanged = form.slug !== baseline.slug
  const debouncedSlug = useDebounce(form.slug, 300)
  useEffect(() => {
    if (debouncedSlug === baseline.slug || !SLUG_RE.test(debouncedSlug)) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (slugChanged && !SLUG_RE.test(form.slug)) {
      toast({
        title: 'Invalid workspace ID',
        description: '3–50 characters: lowercase letters, digits and hyphens.',
        variant: 'destructive',
      })
      return
    }
    if (slugChanged && slugAvailable === false) {
      toast({
        title: 'Workspace ID unavailable',
        description: 'That ID is reserved or already taken — pick another.',
        variant: 'destructive',
      })
      return
    }
    ...
    try {
      ...
      if (prefs.timezone !== basePrefs.timezone) payload.timezone = prefs.timezone
      if (prefs.fiscalYearStartMonth !== basePrefs.fiscalYearStartMonth)
        payload.fiscalYearStartMonth = prefs.fiscalYearStartMonth
      if (prefs.weekStartsOn !== basePrefs.weekStartsOn) payload.weekStartsOn = prefs.weekStartsOn
  ...
  return (
    <SettingsLayout>
      <SectionHead
        title="Organization"
        sub="Your workspace identity, registered address, tax IDs — and workspace-wide defaults."
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <section className="card p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setForm((p) => ({
                      ...p,
                      name,
                      // Founder ask: the workspace ID follows a rename until
                      // the ID is edited by hand — saved only on Save.
                      ...(slugTouched ? {} : { slug: toSlug(name) || p.slug }),
                    }))
                  }}
              ...
              <Field
                label="Workspace ID"
                hint="Used in your CRM email-in address and the admin console — not part of your app URL yet."
              >
                <input
                  ...
                  onChange={(e) => {
                    setSlugTouched(true)
                    set('slug', toSlug(e.target.value) || e.target.value.toLowerCase())
                  }}
                  maxLength={50}
                />
                {slugChanged && (
                  <p className="text-xs" style={{ color: SLUG_RE.test(form.slug) && slugAvailable !== false ? 'var(--yellow)' : 'var(--coral)' }}>
                    {!SLUG_RE.test(form.slug)
                      ? '3–50 chars: lowercase letters, digits, hyphens.'
                      : slugAvailable === false
                        ? 'That ID is reserved or already taken.'
                        : checkSlug.isPending
                          ? 'Checking availability…'
                          : 'Saving updates your CRM email-in address to the new ID (mail to the old one keeps working).'}
                  </p>
                )}
              </Field>
          ...
          <section className="card p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              {isIndia ? (
                <Field label="State" hint={form.gstin ? 'From GSTIN' : undefined}>
                  <select
                    className="input"
                    value={
          ...
          {/* ─── Workspace preferences (round 13/14) ─────────────────────── */}
          <section className="card p-6">
            <div className="mb-4">
              <h2 className="t-h3">Workspace preferences</h2>
              <p className="t-mute mt-1">Defaults that apply across the whole workspace.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Field label="Default timezone" hint="Individual offices can override this under Locations & geofence.">
                <select
                  className="input"
                  value={prefs.timezone}
                  onChange={(e) => setPrefs((p) => ({ ...p, timezone: e.target.value }))}
                >
                  {!(TIMEZONES as readonly string[]).includes(prefs.timezone) && (
              ...
              <Field
                ...
                hint={`${fyLabel(prefs.fiscalYearStartMonth)}. Invoice numbering follows this — changing it mid-year starts a new series.`}
              >
                <select
                  className="input"
                  value={prefs.fiscalYearStartMonth}
                  onChange={(e) => setPrefs((p) => ({ ...p, fiscalYearStartMonth: Number(e.target.value) }))}
                >
                  {MONTHS.map((m, i) => (
              ...
              <Field
                ...
                hint="Timesheet weeks begin on this day — a mid-week change applies from the next week."
              >
                <select
                  className="input"
                  value={prefs.weekStartsOn}
                  onChange={(e) => setPrefs((p) => ({ ...p, weekStartsOn: Number(e.target.value) }))}
                >
                  {WEEKDAYS.map((d, i) => (
