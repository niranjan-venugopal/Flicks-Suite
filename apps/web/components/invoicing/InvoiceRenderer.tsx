 ...
 *
 * `theme` recolors the whole document: 'dark' (app default) or 'light' (the
 * white "document" variant used for printable PDFs). Semantic accents
 * (coral/blue/green) are shared — they read fine on both backgrounds.
 */
...
function Address({ lines, color }: { lines: (string | null | undefined)[]; color: string }) {
  const present = lines.filter(Boolean)
  if (!present.length) return null
  return (
    <div style={{ fontWeight: 600, fontSize: 13, color, lineHeight: 1.5, letterSpacing: '-0.02em' }}>
      {present.map((l, i) => (
        <div key={i}>{l}</div>
      ))}
    </div>
...
export function InvoiceRenderer({
  ...
  const sumLabel: React.CSSProperties = { fontWeight: 600, fontSize: 14, color: t.muted50, letterSpacing: '-0.02em' }
  const sumValue: React.CSSProperties = { fontWeight: 700, fontSize: 14, color: t.text, letterSpacing: '-0.02em' }
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 36 }}>
        <div style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 20, color: t.text, letterSpacing: '-0.02em' }}>
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          <StatusChip status={invoice.status} theme={theme} />
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: 32, marginBottom: 32 }}>
        <div>
          ...
          <div style={{ fontWeight: 700, fontSize: 16, color: t.text, letterSpacing: '-0.02em', marginBottom: 4 }}>
        ...
        <div>
          ...
          <div style={{ fontWeight: 700, fontSize: 14, color: t.text }}>{dateFmt(invoice.invoice_date)}</div>
          {invoice.reference && (
            <>
              <div style={{ ...label, marginTop: 14 }}>Reference</div>
              <div style={{ fontWeight: 700, fontSize: 14, color: t.text }}>{invoice.reference}</div>
            </>
          )}
        </div>
        <div>
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 24 }}>
        <thead>
          <tr style={{ borderBottom: t.headBorder, borderTop: t.headBorder }}>
        ...
        <tbody>
          {line_items.map((l) => (
            <tr key={l.line_number} style={{ borderBottom: t.rowBorder }}>
              <td style={td}>
                <div style={{ fontWeight: 700 }}>{l.item_name}</div>
                {l.description && (
                  <div style={{ fontWeight: 600, fontSize: 12, color: t.muted40, marginTop: 2 }}>{l.description}</div>
                )}
              </td>
              {isDomestic && <td style={{ ...td, color: t.muted60 }}>{l.hsn_sac_code ?? '—'}</td>}
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 28 }}>
        <div style={{ width: 320, display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
            <span style={{ fontWeight: 700, fontSize: 16, color: t.text, letterSpacing: '-0.02em' }}>Total</span>
            <span style={{ fontWeight: 700, fontSize: 24, color: t.text, letterSpacing: '-0.04em' }}>
      ...
      {(invoice.notes || invoice.terms_and_conditions) && (
        <div style={{ borderTop: t.rowBorder, paddingTop: 20, display: 'grid', gap: 16 }}>
          {invoice.notes && (
            <div>
              <div style={label}>Notes</div>
              <div style={{ fontWeight: 600, fontSize: 13, color: t.muted60, lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>
                {invoice.notes}
              </div>
            </div>
          )}
          {invoice.terms_and_conditions && (
            <div>
              ...
              <div style={{ fontWeight: 600, fontSize: 13, color: t.muted50, lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>
                {invoice.terms_and_conditions}
              </div>
            </div>
          )}
