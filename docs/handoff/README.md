...
| Document | Purpose |
|----------|---------|
| [`2026-08-29_Round17_Employment_Terms_Owner_Onboarding_Handoff.md`](./2026-08-29_Round17_Employment_Terms_Owner_Onboarding_Handoff.md) | **Start here.** Round 17 (2026-08-29): probation end / confirmation date / notice period become editable (columns shipped in `0001` but no write path ever existed — invite form, Edit profile, DTOs, plus the six org fields the audit snapshot silently dropped), and owners/admins now get a 4-step onboarding wizard that finishes without the Documents step or HR review, activating them directly (server-derived from the membership role). Also fixes the send-back deep link that 404'd. No migration. |
