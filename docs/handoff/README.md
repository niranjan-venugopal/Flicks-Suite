...
| Document | Purpose |
|----------|---------|
| [`2026-08-26_Round10_Onboarding_UX_Handoff.md`](./2026-08-26_Round10_Onboarding_UX_Handoff.md) | **Start here.** Round 10 (2026-08-26, tester feedback): readable dropdown options on Windows (opaque `option` rule), full state names in the wizard + HR dialog, India-only statutory fields (PAN/Aadhaar/UAN gated by location country; passport for foreign locations, Aadhaar last-4 finally persisted, the fake PF auto-fetch input removed with the honest EPFO answer), and the stuck terms checkbox root-caused (Radix body pointer-events) + fixed with the modal sequence enforced. |
